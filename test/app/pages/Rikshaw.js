...
export default function Rikshaw({ navigation }) {
  const dispatch = useDispatch();
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);

  // Default image for rikshaw
  const defaultImage = require("../../assets/images/rikshaw.png");

  useEffect(() => {
    fetchRikshaws();
  }, []);

  const fetchRikshaws = async () => {
    try {
      const products = await getAllData("products");
      // Filter only Rikshaw type products
      const rikshaws = products
        .filter(product => product.type === "Rikshaw")
        .map(product => ({
          ...product,
      ...
      console.error("Error fetching rikshaws:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <View style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "#f5f5f5" }}>
        ...
        <Text style={{ marginTop: 10, color: "#8e44ad" }}>Loading Rikshaws...</Text>
      </View>
    );
  }
      ...
      {items.length === 0 ? (
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center", padding: 20, marginTop: 50 }}>
          <Text style={{ fontSize: 16, color: "#95a5a6", textAlign: "center" }}>
            No Rikshaws available at the moment
          </Text>
        </View>
      ) : (
        items.map((item) => (
